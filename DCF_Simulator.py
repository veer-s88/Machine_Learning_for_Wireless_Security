...
def dcf_sim():
    ...
    # calculate all statistical information for simulation
...
def transmission_time(backoff_slots, data_rate, control_rate, mac_payload, collision):
    ...
    plcp_preamble = 16e-6  # seconds
    plcp_signal = 4e-6  # seconds
    ...
    data_duration = plcp_preamble + plcp_signal + (service + mac_frame + tail + n_pad)/(data_rate * 1e6)  # seconds

    ack = 14 * 8  # bits
    ack_duration = plcp_preamble + plcp_signal + (service + ack + tail) / (control_rate * 1e6)  # seconds
